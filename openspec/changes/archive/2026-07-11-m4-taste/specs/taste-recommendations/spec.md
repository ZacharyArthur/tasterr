# taste-recommendations Specification (delta)
...
Candidate titles SHALL be scored as `α·cosine(profile, title) + β·quality_prior
+ γ·availability_boost`, with the similarity term dominating (α ≫ β, γ). The
quality prior SHALL derive from TMDB vote statistics; the availability boost
SHALL apply to in-library titles per the media-availability capability and SHALL
degrade to no boost when availability is Unknown. Ranked results SHALL pass a
greedy diversity re-rank penalizing similarity to already-picked titles. Titles
the user has marked `not_interested` SHALL be excluded from every personalized
rail; titles the user has already requested, seeded, or watchlisted SHALL be
excluded from the recommended-for-you rail.
...
#### Scenario: Seerr down means no boost, not no rail

- **WHEN** availability is Unknown while scoring
- **THEN** candidates score without the boost and the rail still returns
...
`POST /api/v1/recommendations/reset` SHALL require a valid session and the
same-origin (CSRF) check, delete only the calling user's signals and profile,
and then re-seed from their Seerr request history. When Seerr is unavailable the
reset SHALL still clear the profile and leave the user on the non-personalized
experience. The response SHALL be secret-free and generic on failure.
