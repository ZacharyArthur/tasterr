...
example using the current `2.1.0` GHCR image, loopback host publication, and the named
...
- **THEN** the linked configuration guide contains every required step and variable
  ...
  upgrade to v2.1, roll back across migration `0006`, and troubleshoot Tasterr
...
- **WHEN** an operator deploys the README's published-image Compose example beside a
  populated `.env`
- **THEN** Compose pulls `ghcr.io/zacharyarthur/tasterr:2.1.0`, publishes it on host
...
The root `SECURITY.md` SHALL identify v2.1.x as the supported stable line, direct
...
- **WHEN** an operator or reporter checks the support table after v2.1 publication
- **THEN** v2.1.x is supported and older or pre-release builds are not

#### Scenario: Public release checks repository security features

- **WHEN** the operator prepares the v2.1 tag
...
#### Scenario: V2.1 live gate is complete

- **WHEN** the operator reaches the v2.1 tag step
...
#### Scenario: V2.1 evidence is complete before tagging

- **WHEN** the operator reaches the `v2.1.0` tag step
- **THEN** `docs/releases/v2.1.0.md` records every branch-verifiable required outcome,
...
The current minor release SHALL use SemVer `2.1.0` in backend/frontend package and
lock metadata and Git tag `v2.1.0`. Its release-readiness change and living specs
...
`2.1.0`, `2.1`, `2`, and `latest` while leaving the SHA candidate unchanged; those
stable aliases, their attestation, and a fresh tagged deployment SHALL be verified
before the immutable GitHub Release. Rollback to v2.0 SHALL use the immutable prior
...
- **WHEN** the v2.1 release-readiness change has passed review and been squash-merged
- **THEN** the operator tags that releasable main commit `v2.1.0`, not an unmerged
...
- **THEN** the operator verifies both architectures, attestation, public visibility,
  anonymous pull, and a disposable clean installation before creating `v2.1.0`
...
- **WHEN** the stable image workflow finishes for `v2.1.0`
- **THEN** the operator confirms `2.1.0`, `2.1`, `2`, `latest`, and the unchanged SHA
...
- **WHEN** any verification fails after `v2.1.0` is published
